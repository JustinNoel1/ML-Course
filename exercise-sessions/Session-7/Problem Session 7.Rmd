...
Generate our data:
```{r}
df = generate_lda_clusters(2,20,1000)
xvals = df$data[,1:20]
yvals = df$data[,21]
```
  ...
  function(z) {
    ...
    for(col in seq(num_features)) {
      for(row in seq(num_to_transform)) {
        z[row,col] = (z[row,col]-means[col])/sqrt(vars[col])
      }
    }
    return(z)
  }
...
```{r}
LogisticClassifier = function(tr_data, tr_labels, rescale = T, add_bias = T, one_hot_encode = T , prior_std_dev = 0.1) {
  num_samples = dim(tr_data)[1]

  #transform our input
  #if(rescale) {
  #  scaler = Scaler(tr_data)
  #  tr_data = scaler(tr_data)
  #}  
  #if(add_bias) {  tr_data = bias(tr_data)  }
  
  num_features = dim(tr_data)[2]
  
  #if(one_hot_encode) {
  #  ohe = OHE(tr_labels)
  #  tr_labels = ohe(tr_labels)
  #}
  ...
  train_counter = 0
  probs = function(x, rescale = rescale, add_bias = add_bias) {
    batch_size = dim(x)[1]
    if(rescale) { x = scaler(x) }
    if(add_bias) { x = bias(x) }
    prbs = exp(x %*% beta)
  ...
  train_on_batch = function(batch_size = 128, learning_rate = 0.1 , verbose = F) {
    ...
    multiplicand = probs(data, rescale = F, add_bias = F)
    ...
    if(verbose) {
      ...
      error = -sum(log(multiplicand%*%t(labels)))/batch_size
      print("Error after ")
      print(train_counter)
      print("is:")
      print(error)
      print("Accuracy is ")
      print(acc_counter / batch_size)
      print((multiplicand%*%t(labels))[1:2,])
    }
  }
  environment()
}
...
ohe = OHE(yvals$y)
ty = ohe(yvals$y)
logc = LogisticClassifier(tc, ty, add_bias = T)
```
```{r}
for(i in 1:1000) {
  logc$train_on_batch(batch_size = 512, verbose = T,learning_rate = 0.0001)
  print(logc$beta[1,])
}
```
