...
We will check it on 10000 entries to assess the accuracy and then look at a truth table to see what kind of errors our classifier makes.
```{r}
test <- (tiny %>% select(-Author))[14580:19579,]
preds <- predict(model, newdata = test)
sum(preds == tiny$Author[14580:19579])
table(preds, tiny$Author[14580:19579])
```
First the good news, 60.94% on a dataset is significantly better than randomly guessing (run a hypothesis test to check this). If we got a similar performance on the test set, then this submission would rate us 47th out of 227 entries as of the time of this writing. 
...
sum(preds_lc == tiny$Author[14580:19579])
table(preds_lc, tiny$Author[14580:19579])
```

Okay, so we ran this through a logistic classifier and now we got our accuracy up to 65.28%. Such a submission would get us into the top 20. Not bad for our second shot. Admittedly, it takes more and more effort to move up from here, but this looks like a reasonable track. 
