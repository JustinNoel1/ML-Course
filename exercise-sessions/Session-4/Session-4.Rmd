...
Here we will code a linear regression function excluding the linear algebra part. While doing the linear algebra would also be instructive, it is not what we are trying to do here. Also linear algebra packages are typically crazy fast and any of our implementations would be drastically slower.

The subtle part of this is getting the algorithm to work in the matrix is ill-conditioned. We will use the [QR decomposition](https://en.wikipedia.org/wiki/QR_decomposition). Then we have to compare this with what [R does](https://stat.ethz.ch/R-manual/R-devel/library/base/html/qr.html). 
...
linreg = function(X,Y) {
  ...
  beta = qr.solve(t(newR) %*% newR) %*% t(newR) %*% t(qr.Q(decomp)) %*% Y
  function(A) A[,1:decomp$rank]%*% beta
}
```
...
freg = function(X,Y, n){ 
  ...
  function(A) {
     sins = outer(A,1:n,function(a,b) sin(a*b))
